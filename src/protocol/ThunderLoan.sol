    ...
    OracleUpgradeable
{
    ...
    // @audit-info : it should be a constant or immutable
    uint256 private s_feePrecision; // @audit-gas : this should be constant / Immutable
    ...
    function deposit(
        ...
        // @audit-high : we shouldn't be updating exchangeRate here!!
    ...
    function flashloan(
        ...
        IERC20 token, // erc20 to borrow
        uint256 amount, // the amount to borrow
        bytes calldata params // the parameters to call the reciever address with
    ) external revertIfZero(amount) revertIfNotAllowedToken(token) {
        AssetToken assetToken = s_tokenToAssetToken[token];
        ...
        // e making sure reciever address is a smart contract.
    ...
    function getCalculatedFee(
        ...
        // 3 * 0.2 WETH !!
        // 0.6 WETH * 0.3e18
        // @audit-high : if the fee is going to be in the token, then the value should reflect it.
